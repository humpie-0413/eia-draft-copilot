...
- **narrative_generator.py:525**: `IndicatorStats.max`/`.min` → `.max_value`/`.min_value` 속성명 불일치 수정
  - 교통 섹션 서술문 생성 시 최대/최소 교통량 범위 출력 코드에서 크래시 발생
  - `IndicatorStats` dataclass의 실제 필드명은 `max_value`/`min_value`인데 `.max`/`.min`으로 접근
- **demo_full_scenario.py**: httpx 타임아웃 120s → 300s 증가
  - export/preview 엔드포인트가 scaffold + statistics + standards + QA를 재계산하여 대용량 데이터(3200건+) 처리 시 타임아웃

#### 2. 통합 데모 전체 실행 성공 (11단계)
- 단계 1: 프로젝트 생성 ✅
- 단계 2: 데이터 수집 — API 7/9 성공 + 수동 2종, 총 3,205건 ✅
- 단계 3: 유사사례 등록 3건 + 매칭 ✅ (최고 유사도 0.58)
- 단계 4: 섹션 플래너 — 완료 4개, 부분 4개 ✅
- 단계 4.5: 법령 반영 검증 — 평가 범위/필수 섹션/서술문 법적 근거 확인 ✅
- 단계 5: 통계 엔진 — 수치 데이터 2,545건 산출 ✅
- 단계 6: 환경기준 비교 — 소음 기준 초과 2건 검출 ✅
- 단계 7: 초안 뼈대 + 서술문 — 11개 섹션 모두 서술문 생성, 예측 3종 포함 ✅
- 단계 7.5: 영향 예측 — 대기 확산 24건, 소음 전파 12건, 수질 혼합 5건 ✅
- 단계 8: LLM 보강 — adapter=none 확인 (설계 의도대로 건너뜀) ✅
- 단계 9: QA — critical 2건, warning 10건, info 10건 (총 22건) ✅
- 단계 10: Export — critical 이슈로 정상 차단 + 문서 구조 미리보기 ✅
- 단계 11: 최종 현황 보고 ✅

#### 3. 커넥터 가동률
- 성공: 7/9 (78%)
- 실패: soil_info (서버 장애), cultural_heritage (강남구 반경 내 문화재 없음)
- 실제 API 데이터: 3,197건 | 수동 입력: 8건

#### 4. 테스트 612개 전체 통과

### 주요 파일
- `backend/app/services/narrative_generator.py` — max/min 속성명 수정
- `scripts/demo_full_scenario.py` — httpx 타임아웃 증가
...
## Phase Demo-3: 3종 시나리오 데모 스크립트 ✅

### 완료 항목

#### 시나리오 1: 국도 우회도로 건설 — 경기 양평 (`scripts/demo_road_yangpyeong.py`)
- 사업유형: road, 양평군 양평읍 (127.49°E, 37.49°N)
- 도로 형태 폴리곤 (폭 200m, 길이 2km)
- 커넥터 9종: 에어코리아(양평), 수질(한강 수계), 기상(양평 202), V-world, 토지이용규제, 문화재, 교통량, 폐기물, 토양
- 수동 데이터: 소음·진동(3), 생태(5, 보호종 수달·원앙), 경관(3, 양수리 두물머리)
- 유사사례 3건: 포천 국도 확장, 원주 우회도로, 충주 우회도로
- 예측: 소음 전파(선음원 75dB), 대기 확산(도로 분진 H=5m), 수질 혼합

#### 시나리오 2: 택지개발 신도시 — 세종시 (`scripts/demo_housing_sejong.py`)
- 사업유형: housing, 세종시 조치원읍 (127.0°E, 36.6°N)
- ~1km x 1km 사각형 폴리곤
- 커넥터 9종: 에어코리아(세종), 수질(금강 수계), 기상(대전 133), V-world, 토지이용규제, 문화재, 교통량, 폐기물, 토양
- 수동 데이터: 소음·진동(3), 생태(5, 보호종 0)
- 유사사례 3건: 동탄 신도시, 오송 택지개발, 다산 택지개발
- 예측: 대기 확산(건설 분진 PM10=1.0), 소음 전파(건설 장비 90dB), 수질 혼합(방류량 0.03)

#### 시나리오 3: 화력발전소 증설 — 충남 보령 (`scripts/demo_powerplant_boryeong.py`)
- 사업유형: power_plant, 보령시 오천면 (126.5°E, 36.3°N)
- ~500m x 500m 사각형 폴리곤
- 커넥터 9종: 에어코리아(보령), 수질(금강 하류), 기상(보령 235), V-world, 토지이용규제, 문화재, 교통량, 폐기물, 토양
- 수동 데이터: 소음·진동(3), 생태(5, 간척지), 기후 보강(3, 기상청 실패 시만)
- 유사사례 3건: 서천 화력, 영광 원자력, 삼척 화력
- 예측: 대기 확산(굴뚝 80m, PM10=2.0/NO2=5.0/SO2=3.0), 소음 전파(터빈 100dB), 수질 혼합(방류량 0.05)

#### 공통 특징
- 기존 demo_full_scenario.py 미수정
- 11단계 동일 구조 (프로젝트 생성 → 수집 → 유사사례 → 섹션 → 법령 → 통계 → 기준비교 → 서술문 → 예측 → LLM → QA → GIS → Export → 요약)
- 더미 데이터 절대 사용 금지 원칙 준수
- 각 단계별 "API 자동수집" vs "수동입력" vs "미수집" 구분 표시
- output/ 폴더에 시나리오별 DOCX + PDF + GIS 도면 생성

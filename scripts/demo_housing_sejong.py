# -*- coding: utf-8 -*-
"""데모 시나리오 2: 택지개발 신도시 (세종시).

시나리오: "세종시 신도시 택지개발사업 환경영향평가"
...
전체 흐름:
  ...
  6. 환경기준 비교 실행
  7. 초안 뼈대 + 서술문 생성
  7.5. 영향 예측 실행 (대기 확산, 소음 전파, 수질 혼합)
  8. LLM 보강 (선택)
  9. QA 실행
  9.5. GIS 도면 생성
  10. DOCX + PDF export
...
async def step1_create_project(client: httpx.AsyncClient) -> str | None:
    ...
    data = {
        "name": "세종시 신도시 택지개발사업 환경영향평가",
        "description": "세종시 조치원읍 일대 신도시 택지개발에 따른 환경영향평가",
        "project_type": "housing",
        "geometry": SEJONG_POLYGON,
    }
...
async def step2_collect_data(client: httpx.AsyncClient, project_id: str) -> dict:
    ...
    await collect_connector(
        "keco_air",
        "2-a. 에어코리아 대기질 커넥터 — 측정소: 아름동",
        {"station_name": "아름동", "data_term": "DAILY"},
    )

    # 2-b. 수질 커넥터 — 금강 수계
    await collect_connector(
        "water_info",
        "2-b. 수질 커넥터 — 금강 수계 측정지점 (3008A20)",
        {"year": "2024", "pt_no": "3008A20"},
    )
    ...
    # 2-d. 기상청 ASOS — 대전 관측소 (133)
    await collect_connector(
        "kma_weather",
        "2-d. 기상청 ASOS 기후 커넥터 — 대전(133)",
        {"stn_id": "133", "start_dt": "20240701", "end_dt": "20241231"},
    )

    # 2-e. V-world 토지이용
    await collect_connector(
        "vworld_land_use",
        "2-e. V-world 토지이용 커넥터 — 세종시 중심점",
        {"lng": "127.2494", "lat": "36.5101"},
    )

    # 2-f. 토지이용규제정보
    await collect_connector(
        "land_use_regulation",
        "2-f. 토지이용규제정보 커넥터 — 세종시",
        {"area_cd": "36110", "ucodes": ["UQA100"], "land_use_nm": "건축"},
    )

    # 2-g. 국가유산청 문화재
    await collect_connector(
        "cultural_heritage",
        "2-g. 국가유산청 문화재 커넥터 — 세종 인근",
        {"lng": "127.2494", "lat": "36.5101"},
    )
    ...
    noise_evidences = [
        ...
        {"category": "noise_vibration", "indicator": "진동_Lv_주간", "value": "48.0", "numeric_value": 48.0, "unit": "dB(V)", "observed_at": "2025-11-05T10:00:00"},
    ]
    ...
    ecology_evidences = [
        ...
        {"category": "ecology", "indicator": "법정보호종", "value": "0", "numeric_value": 0.0, "unit": "종", "observed_at": "2025-10-10T00:00:00"},
        {"category": "ecology", "indicator": "녹지자연도", "value": "5등급", "observed_at": "2025-10-10T00:00:00"},
        {"category": "ecology", "indicator": "비오톱_유형", "value": "도시녹지", "observed_at": "2025-10-10T00:00:00"},
    ]
    ...
    print("    ├───────────────────────┼────────┼───────┼──────────────────────────────────┤")
    manual_labels = {"noise_vibration": "소음·진동", "ecology": "생태"}
...
SIMILAR_CASES = [
    {
        "name": "경기도 화성시 동탄 신도시 환경영향평가",
        "description": "화성시 동탄면 일대 대규모 택지개발 사업",
        "project_type": "housing",
        "location": {"type": "Point", "coordinates": [127.1, 37.2]},
        "area_sqm": 500000.0,
        "summary": "대규모 택지개발. 대기질 및 교통 영향 중점 평가, 녹지축 보전 권고.",
        "key_findings": {
            "대기질": "건설 중 비산먼지 및 차량 배기가스 관리",
            "교통": "교통량 증가에 따른 도로 확충 필요",
            "생태": "녹지축 단절 방지 계획 수립",
        },
        "evidence_categories": ["air_quality", "noise_vibration", "ecology", "traffic"],
    },
    {
        "name": "충북 청주시 오송 택지개발 환경영향평가",
        "description": "청주시 흥덕구 오송읍 일대 택지개발",
        "project_type": "housing",
        "location": {"type": "Point", "coordinates": [127.1, 36.6]},
        "area_sqm": 300000.0,
        "summary": "오송생명과학단지 인접 택지개발. 수질 및 우수관리 중점 평가.",
        "key_findings": {
            "수질": "미호천 수질 영향 최소화 대책",
            "대기질": "건설 분진 저감 계획",
            "소음": "인접 주거지 공사소음 관리",
        },
        "evidence_categories": ["water_quality", "air_quality", "noise_vibration"],
    },
    {
        "name": "경기도 남양주시 다산 택지개발 환경영향평가",
        "description": "남양주시 다산동 일대 택지개발 사업",
        "project_type": "housing",
        "location": {"type": "Point", "coordinates": [127.2, 37.6]},
        "area_sqm": 400000.0,
        "summary": "한강변 택지개발. 수질 및 경관 영향 중점 평가, 완충녹지 설정.",
        "key_findings": {
            "수질": "한강 수질 보전을 위한 우수처리 시설",
            "경관": "한강변 스카이라인 관리",
            "생태": "한강 수변 생태 보전",
        },
        "evidence_categories": ["water_quality", "ecology", "landscape"],
    },
]
...
async def step4_5_regulation_check(client: httpx.AsyncClient, project_id: str) -> dict:
    ...
    sub_banner("4.5-a. 평가 범위 조회 (housing)")
...
async def step7_5_prediction(client: httpx.AsyncClient, project_id: str) -> dict:
    """대기 확산(건설 분진), 소음 전파(건설 장비), 수질 혼합 예측 실행."""
    ...
    # 대기 확산 예측 (건설 분진)
    sub_banner("7.5-a. 대기 확산 예측 (건설 분진, H=5m)")
    result = await api_call(
        client, "POST",
    ...
    # 소음 전파 예측 (건설 장비, Lw=90dB)
    sub_banner("7.5-b. 소음 전파 예측 (건설 장비, Lw=90dB)")
    result = await api_call(
        client, "POST",
    ...
    # 수질 혼합 예측
    sub_banner("7.5-c. 수질 혼합 예측 (완전혼합, 방류량 0.03 m³/s)")
    result = await api_call(
        client, "POST",
...
async def step10_export(client: httpx.AsyncClient, project_id: str) -> dict:
    ...
    if content and isinstance(content, bytes):
        docx_path = str(OUTPUT_DIR / f"demo_housing_sejong_{timestamp}.docx")
    ...
    if content and isinstance(content, bytes):
        pdf_path = str(OUTPUT_DIR / f"demo_housing_sejong_{timestamp}.pdf")
...
def step11_summary(
    ...
    """전체 데모 결과 요약."""
    banner("단계 11: 최종 결과 요약 — 세종시 신도시 택지개발사업")

    print(f"    프로젝트 ID: {project_id}")
    print(f"    프로젝트명: 세종시 신도시 택지개발사업 환경영향평가")
    print(f"    사업유형: housing (주거단지)")
    ...
    print(f"  │ 실제 API: {api_count}건 | 수동 입력: {manual_count}건 (소음·생태)")
    print()
    ...
    print(f"  │ 실행 모델: {len(models_run)}/3 ({', '.join(models_run)})")
    print()
...
async def main():
    print("╔══════════════════════════════════════════════════════════════════════╗")
    print("║  EIA Draft Copilot — 데모 시나리오 2                               ║")
    print("║  세종시 신도시 택지개발사업 환경영향평가                               ║")
    print("║  사업유형: housing | 원칙: 실제 API 데이터만 사용                     ║")
    print("╚══════════════════════════════════════════════════════════════════════╝")
    print()
